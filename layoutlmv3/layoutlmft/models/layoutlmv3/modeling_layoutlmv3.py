...
class LayoutLMv3Embeddings(nn.Module):
    ...
    def forward(
        ...
        if token_type_ids is None:
            token_type_ids = torch.zeros(input_shape, dtype=torch.long).to(self.position_ids.device)
...
class LayoutLMv3PreTrainedModel(PreTrainedModel):
    ...
    config_class = LayoutLMv3ConfigOB
...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    ...
    def forward(
        ...
        if not self.image_only:
            if bbox is None:
                bbox = torch.zeros(tuple(list(input_shape) + [4]), dtype=torch.long).to(device)
        ...
        if images is not None:
            ...
            if self.detection:
                visual_attention_mask = torch.ones((batch_size, visual_emb.shape[1]), dtype=torch.long).to(device)
                if self.image_only:
                    attention_mask = visual_attention_mask
                else:
                    attention_mask = torch.cat([attention_mask, visual_attention_mask], dim=1)
            elif self.image_only:
                attention_mask = torch.ones((batch_size, visual_emb.shape[1]), dtype=torch.long).to(device)
            ...
            if self.config.has_relative_attention_bias or self.config.has_spatial_attention_bias:
                ...
                visual_position_ids = torch.arange(0, visual_emb.shape[1], dtype=torch.long).to(device).repeat(
                    batch_size, 1)
                if self.image_only:
                    final_position_ids = visual_position_ids
                else:
                    position_ids = torch.arange(0, input_shape[1]).to(device).unsqueeze(0)
        ...
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(attention_mask, None).to(device)
